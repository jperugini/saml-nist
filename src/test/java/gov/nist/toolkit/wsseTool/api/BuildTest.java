...
public class BuildTest extends BaseTest {
	...
	private static final Logger log = Logger.getLogger(BuildTest.class);
	...
	@Test
	public void buildToFileTest() throws GenerationException {
		
		Document xml = new OpenSamlWsseSecurityGenerator().generateWsseHeader(context);
		
		log.debug("header to validate : \n {}" + MyXmlUtils.DomToString(xml) );
		
		File file = new File("src/test/resources/sets/debuggingSession/generatedAsFile.xml");
		MyXmlUtils.DomToFile(xml, file.getName());
	}
