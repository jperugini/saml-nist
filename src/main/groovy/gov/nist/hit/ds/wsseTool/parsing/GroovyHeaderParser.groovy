...
public class GroovyHeaderParser {

	private static final Logger log = Logger.getLogger(GroovyHeaderParser.class)

	GroovyHeader header

	public GroovyHeaderParser(GroovyHeader gHeader){
		header = gHeader
	}
	public void parse() throws ParseException {
		try{
			...
			throw new ParseException("Could not parse the input message.", e)
		}
	}
	...
	public void assertionStructure(){
		...
		def issuers = header.map.assertion.children().findAll{
			it.name() == 'Issuer'
		}
		def a_signatures = header.map.assertion.children().findAll{
			it.name() == 'Signature'
		}
		def subjects = header.map.assertion.children().findAll{
			it.name() == 'Subject'
		}
		def attributeStatements = header.map.assertion.children().findAll{
			it.name() == 'AttributeStatement'
		}
		def authnStatements = header.map.assertion.children().findAll{
			it.name() == 'AuthnStatement'
		}
		def authzDecisionStatements = header.map.assertion.children().findAll{
			it.name() == 'AuthzDecisionStatement'
		} //optional
		...
		header.map.attributeStatements = attributeStatements
