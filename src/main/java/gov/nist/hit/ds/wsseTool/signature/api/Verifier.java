...
public class Verifier {
	...
	static KeyInfoFactory keyInfoFac = KeyInfoFactory.getInstance("DOM");
	 ...
	 */
	public boolean verifyTimestampSignature(OpenSamlSecurityHeader header) throws KeyException {

		// get the subjectConfirmationData
		SubjectConfirmation conf = header.getAssertion().getSubject().getSubjectConfirmations().get(0);

		assertNotNull("Subject confirmation shall be present.", conf);

		org.opensaml.saml2.core.SubjectConfirmationData data = conf.getSubjectConfirmationData();

		assertNotNull("Subject confirmation data shall be present.", data);

		// from spec, keyInfo must be the only child of subjectConfirmationData
		// TODO (verify assumption)
		org.opensaml.xml.signature.KeyInfo keyInfo = (KeyInfo) data.getOrderedChildren().get(0);

		assertNotNull("Key info element for subject confirmation shall be present.", keyInfo);
		...
		assertNotNull("Key info element for subject confirmation shall contain a unique public key.", pkey);
	...
	public boolean verifySamlAssertionSignature(OpenSamlSecurityHeader header) throws KeyException {
	...
	/**
	 * @deprecated (replaced by verifyTimestampSignature) Verify the timestamp
	 *             signature. We assume the document is well-formed and valid.
	 *             If any problem occurs in the parsing, it should be catched by
	 *             earlier tests
	 * 
	 * @param header
	 *            : the opensaml representation of this document
	 * @return true if valid
	 */
	public boolean verifyTimestampSignatureWithDOM(OpenSamlSecurityHeader header) throws MarshalException,
			KeyException, XMLSignatureException {
		...
		javax.xml.crypto.dsig.keyinfo.KeyInfo DOMkeyInfo = keyInfoFac.unmarshalKeyInfo(keyInfoElt);
		...
		// get the signature
		// TODO change : might not always be in 3rd position (Use DOM document
		// instead?)
	 ...
	 */
	private boolean verify(Element element, Key key) throws MarshalException, XMLSignatureException {

		DOMValidateContext valContext = new DOMValidateContext(KeySelector.singletonKeySelector(key), element);
		// TODO check! seems to work without giving the signature namespace!
		// valContext.putNamespacePrefix(javax.xml.crypto.dsig.XMLSignature.XMLNS,
		// NwhinNamespace.DS.prefix());
