...
public class OpenSamlWsseSecurityGenerator {
	...
	private static final Logger log = Logger.getLogger(OpenSamlWsseSecurityGenerator.class);
	...
	private void updatePatientId(Document sec) throws GenerationException, XPathFactoryConfigurationException {
		...
		try {
			node = (Node) xp.evaluate("//*[@Name='urn:oasis:names:tc:xacml:2.0:resource:resource-id']",
					sec.getDocumentElement(), XPathConstants.NODE);
	...
	private void updateHomeCommunityId(Document sec) throws GenerationException, XPathFactoryConfigurationException {
...
//		AXIOMXPath xpathExpression = new AXIOMXPath("//*[@Name='urn:nhin:names:saml:homeCommunityId']");
//		OMElement node = (OMElement)xpathExpression.selectSingleNode(om);
//		OMElement value = (OMElement) node.getChildrenWithLocalName("AttributeValue").next();
//		value.setText(homeCommunityId);
		...
		log.debug("Replace homeCommunityId : {} in template by homeCommunityId : {} " + attrValue.getTextContent() +
				homeCommunityId);
	}
	...
	private void updateTimeInfo(Document sec) {
		try {
			...
			nodes = (NodeList) xp.evaluate("//@*", sec.getDocumentElement(), XPathConstants.NODESET);
			...
			for (int i = 0; i < nodes.getLength(); i++) {
				...
				if (name.equals("NotOnOrAfter")) {
					...
					nodes.item(i).setNodeValue(created.minusHours(1).toString());
				} else if (name.equals("IssueInstant")) {
					nodes.item(i).setNodeValue(created.minusMinutes(5).toString());
				} else if (name.equals("AuthnInstant")) {
					nodes.item(i).setNodeValue(created.minusMinutes(7).toString());
				}
			}
		} catch (XPathExpressionException e) {
	...
	private void updateTimestamp(OpenSamlSecurityHeader sec) {
		log.info("update time stamp dates : {}" + sec.getTimestamp().getElementQName().getLocalPart());
	...
	private void signWsseHeader(OpenSamlSecurityHeader sec, Document securityHeader, KeystoreAccess access)
			throws GenerationException {

		String assertURI = sec.getAssertion().getElementQName().getNamespaceURI();
		Element assertion = (Element) securityHeader.getElementsByTagNameNS(assertURI, "Assertion").item(0);

		// Find subjectConfirmationData and add keyinfo
		String confURI = sec.getSubject().getElementQName().getNamespaceURI();
		Node subjectConfirmationData = assertion.getElementsByTagNameNS(confURI, "SubjectConfirmationData").item(0);
		...
		Node subject = assertion.getElementsByTagNameNS(subURI, "Subject").item(0);

		// FIX FOR ALL JAVA ABOVE JAVA 6 update 37. The ID must be explicitly
		// set.
		...
		Signer.signTimeStamp(securityHeader.getDocumentElement(), timestampIdRef, samlIdRef, access.keyPair);
