...
public class Signer {
	 ...
	 */
	public static void signTimeStamp(Node document, String id, String refId,
			KeyPair keyPair) throws GenerationException {
		try {
			...
			sign(document, null, id, keyPair.getPrivate(), keyInfo, transformList);
		} catch (Exception e) {
			throw new GenerationException(e);
		}
	}
	...
	private static void sign(Node xml, Node nextSibling, String id, PrivateKey privateKey, KeyInfo keyInfo, List<Transform> transformList)
		...
		log.info("try to add signature to DOM element : {} " + xml.getLocalName());
		...
		log.info("successfully added signature to DOM element : {} " + xml.getLocalName());
